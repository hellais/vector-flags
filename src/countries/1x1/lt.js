import React from 'react'

const FlagLT = (props) => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" height={512} width={512} id="flag-icon-css-lt" viewBox="0 0 512 512" {...props}>
      <g fillRule="evenodd" transform="scale(.51314 1.0322)" strokeWidth="1pt">
        <rect transform="scale(.93865 .69686)" rx={0} ry={0} height="708.66" width={1063} fill="#007308" />
        <rect transform="scale(.93865 .69686)" rx={0} ry={0} height="236.22" width={1063} y="475.56" fill="#bf0000" />
        <path fill="#ffb300" d="M0 0h997.77v164.61H0z" />
      </g>
    </svg>
    
  );
}

export default FlagLT
